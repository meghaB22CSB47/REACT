  ...
  return (
    <div className="patient-dashboard">
      ...
      <div className="patient-container">
        ...
        <select id="status" value={status} onChange={(e) => setStatus(e.target.value)} className="patient-select">
      ...
      {status === "Accepted" && (
        <div className="patient-container">
          ...
          {doctors.length > 0 ? doctors.map((doctorId) => (
            <div key={doctorId} className="patient-doctor-actions-row">
              <span className="patient-doctor-id-display">Doctor ID: {doctorId}</span>
              <button className="patient-doctor-button patient-view-button" onClick={() => viewHistory(doctorId)}>View History</button>
              <button className="patient-doctor-button patient-revoke-button" onClick={() => handleAction('Revoke', doctorId)}>Revoke</button>
            </div>
      ...
      {status === "Pending" && (
        <div className="patient-container">
          ...
          {pendingDoctors.length > 0 ? pendingDoctors.map((doctorId) => (
            <div key={doctorId} className="patient-doctor-actions-row">
              <span className="patient-doctor-id-display">Doctor ID: {doctorId}</span>
              <button className="patient-doctor-button patient-accept-button" onClick={() => handleAction('Accepted', doctorId)}>Accept</button>
              <button className="patient-doctor-button patient-revoke-button" onClick={() => handleAction('Rejected', doctorId)}>Reject</button>
            </div>
